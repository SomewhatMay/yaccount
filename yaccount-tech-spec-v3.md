...
- **Feedback = `sonner` toasts, bottom-right,** in the interface's voice (see §12.6). **(Post-M11 Phase 1)** Below `lg`, their bottom offset clears the fixed 56px tab bar, the safe-area inset and a 16px gap; desktop remains bottom-right. Every create/update/delete confirms with a toast.
