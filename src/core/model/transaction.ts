...
export const TransactionSchema = z
  .object({
    ...
    // The schedule date this row satisfies. It may differ from `date` when a
    // manually entered payment happened early/late; null means no exact link.
    // Default preserves pre-field rows during import/replay.
    recurring_occurrence_date: zIsoDate.nullable().default(null),
  ...
  .refine((t) => t.recurring_occurrence_date === null || t.recurring_rule_id !== null, {
    message: "a recurring occurrence date needs a recurring rule",
    path: ["recurring_occurrence_date"],
  });
...
export function makeTransaction(input: {
  ...
  return TransactionSchema.parse({
    ...
    recurring_occurrence_date: input.recurring_occurrence_date ?? null,
...
/** Exact schedule identity, falling back for rows generated before this field. */
export function recurringOccurrenceDate(row: Transaction): string {
  return row.recurring_occurrence_date ?? row.date;
}
